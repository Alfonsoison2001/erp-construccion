...
export default function BudgetVsPaid({ categories, totalBudget, totalPaid, isAdmin = true }) {
        ...
        <table className="w-full">
          <thead>
            <tr className="bg-gray-50 border-b border-gray-200">
              ...
              {isAdmin && <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Presupuesto</th>}
              <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Pagado</th>
              {isAdmin && <th className="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Pendiente</th>}
              {isAdmin && <th className="px-4 py-3 text-xs font-medium text-gray-500 uppercase w-48">Avance</th>}
            </tr>
          </thead>
          <tbody>
            {categories.map(cat => {
            ...
            <tr className="bg-gray-50 font-semibold border-t-2">
              ...
              {isAdmin && <td className="px-4 py-3 text-right">{formatCurrency(totalBudget)}</td>}
              <td className="px-4 py-3 text-right">{formatCurrency(totalPaid)}</td>
              {isAdmin && <td className="px-4 py-3 text-right">{formatCurrency(totalBudget - totalPaid)}</td>}
              {isAdmin && (
...
function CategoryRow({ cat, pending, pct, isExpanded, isAdmin, onToggle }) {
  return (
    <>
      <tr
      ...
      >
        ...
        {isAdmin && <td className="px-4 py-2.5 text-right text-sm">{formatCurrency(cat.budget_total)}</td>}
        <td className="px-4 py-2.5 text-right text-sm">{formatCurrency(cat.paid_total)}</td>
        {isAdmin && <td className="px-4 py-2.5 text-right text-sm">{formatCurrency(pending)}</td>}
        {isAdmin && (
          <td className="px-4 py-2.5">
            <ProgressBar
      ...
      {isExpanded && cat.concepts.map(con => {
        ...
        return (
          <tr key={con.concept_id} className="border-b border-gray-50 bg-gray-50/50">
            ...
            {isAdmin && <td className="px-4 py-2 text-right text-sm text-gray-600">{formatCurrency(con.budget_total)}</td>}
            <td className="px-4 py-2 text-right text-sm text-gray-600">{formatCurrency(con.paid_total)}</td>
            {isAdmin && <td className="px-4 py-2 text-right text-sm text-gray-600">{formatCurrency(conPending)}</td>}
            {isAdmin && (
